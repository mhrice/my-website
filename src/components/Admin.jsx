...
getEmails(){
  var items = this.state.emails.map((email)=>{
      return (
        ...
        <h5>Name: {email.firstName} {email.lastName}</h5>
        <h5>Email: {email.email}</h5>
        <h5><RaisedButton onClick={()=>{this.showMessage(email.message, email.firstName, email.lastName)}} className="admin-message">Message</RaisedButton></h5>
  ...
  render(){
    return(
      <div className="admin-container" onClick={()=>{this.closeMessage()}}>
      <h1 className="admin-header">Emails <RaisedButton label="Logout" onClick={()=>{this.logout()}} className="admin-logout" primary={true}> </RaisedButton></h1>
